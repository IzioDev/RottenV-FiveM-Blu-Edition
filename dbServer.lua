...
Citizen.CreateThread(function()
		...
		MySQL.Async.fetchAll('SELECT * FROM players where steamid="'..GetPlayerIdentifier(client,0)..'"', {}, function(player) 
			if player[1] then
				if player[1].steamid == GetPlayerIdentifier(client,0) then
					TriggerClientEvent("loadPlayerIn", client, player[1].x, player[1].y, player[1].z, player[1].hunger, player[1].thirst, player[1].weapons, player[1].inv, player[1].health)
				end
			else
	...
	AddEventHandler("SavePlayerData", function(client,posX,posY,posZ,hunger,thirst,weapons,inv,health)
		MySQL.Async.execute('UPDATE players SET x="'..posX..'", y="'..posY..'", z="'..posZ..'", hunger="'..hunger..'", thirst="'..thirst..'", weapons="'..weapons..'", inv="'..inv..'", health="'..health..'" WHERE steamid="'..GetPlayerIdentifier(client,0)..'"', {}, function() end)
	end)
end)
